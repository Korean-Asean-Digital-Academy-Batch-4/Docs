...
| `Not authorized to perform sts:AssumeRoleWithWebIdentity` | **Organisasi mengaktifkan *custom subject claim*, sehingga `sub` bukan format baku.** Terjadi pada `Korean-Asean-Digital-Academy-Batch-4`: `sub` yang dikirim memuat pengenal numerik organisasi dan repositori, misalnya `repo:ORG@307598735/backend@1325655840:ref:refs/heads/main` | Jalankan langkah pencetak klaim pada Bagian 4, **salin nilai `sub` yang tercetak apa adanya** ke trust policy. Jangan menyusunnya sendiri dari nama repositori |
| `Not authorized to perform sts:AssumeRoleWithWebIdentity` | **Pengenal akun pada trust policy masih berupa teks pengganti**, sehingga menunjuk identity provider yang tidak ada | Roles → `edutrack-gha-backend` → tab **Trust relationships** → **Edit trust policy**. Kolom **Trusted entities** pada daftar Roles harus menampilkan angka |
...
| 7 Agustus 2026 | Penyebab sungguhan dimasukkan ke tabel diagnosa Bagian 6: organisasi mengaktifkan *custom subject claim*, sehingga `sub` memuat pengenal numerik organisasi dan repositori dan tidak akan pernah cocok dengan format baku. Ini yang menghabiskan empat percobaan, dan akan menimpa role frontend dengan pengenal repositori yang berbeda |
