...
| A-07 | Katalog kesalahan §10 tidak memuat kode bagi **tiga keadaan status rapor** yang muncul saat A7 diturunkan: finalisasi atas kelas yang sudah final, distribusi atas kelas yang belum final, dan finalisasi maupun distribusi atas kelas yang belum memiliki satu pun siswa. Ketiganya konflik keadaan, bukan bentuk permintaan yang salah | Sementara ini keduanya yang pertama dijawab `409 RAPOR_TERKUNCI` dan yang ketiga `400 PERMINTAAN_TIDAK_SAH`. Usul: tambahkan satu kode `STATUS_RAPOR_TIDAK_SESUAI` (409) ke §10, atau sahkan pemetaan sementara ini apa adanya |
| A-08 | `rapor_mapel.kehadiran_persen` bertipe `NOT NULL`, tetapi tidak ada dokumen yang menetapkan nilainya ketika **belum ada satu pun sesi dibuka** pada satu mata pelajaran. Kueri [SCHEMA §8.3](SCHEMA.md) tidak menghasilkan baris sama sekali pada keadaan itu | Dicatat **100** oleh `domain/rapor.ts`, dengan alasan P16 dan AC-29 menyatakan hanya Alpa yang mengurangi persentase, dan tanpa sesi tidak ada satu pun Alpa. Nol adalah jawaban yang salah karena berarti siswa selalu alpa. Perlu disahkan pihak sekolah bersama **V5** |
| A-09 | Berkas rapor tidak memuat **identitas sekolah** — nama, NPSN, alamat, dan logo. Keempatnya lazim ada pada rapor resmi, tetapi bukan data siswa melainkan tetapan pemasangan, dan tidak ada dokumen yang menetapkan tempatnya. [ARCHITECTURE §11.3](ARCHITECTURE.md) mencatatnya sebagai sengaja belum dimuat | Tetapkan sebagai konfigurasi pemasangan, bukan entitas basis data — sejalan dengan `install.sh` tunggal pada CK-15. Menyimpannya sebagai baris tabel berarti tabel bertepat satu baris selamanya. Perlu diputuskan sebelum rapor pertama dibagikan kepada orang tua |
